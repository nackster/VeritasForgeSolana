...
use anchor_spl::{
    token::{
        self, Token, TokenAccount, InitializeMint2, MintTo, SetAuthority, AuthorityType,
    },
...
declare_id!("AFoRGexxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
...
pub mod arenaforge_mvp {
    ...
    pub fn initialize_global(
        ...
        global.bump = *ctx.bumps.get("global").ok_or(ArenaErr::MissingBump)?;
        global.reserve_bump = *ctx
            .bumps
            .get("reserve_vault")
            .ok_or(ArenaErr::MissingBump)?;
        Ok(())
    }
    ...
    pub fn register_forge(ctx: Context<RegisterForge>) -> Result<()> {
        ...
        forge.bump = *ctx.bumps.get("forge").ok_or(ArenaErr::MissingBump)?;
        Ok(())
    }
    ...
    pub fn create_arena(
        ...
        kind: ArenaKind,
        reward_bps_from_reserve: u16, // portion of reserve paid to winner on resolve
        ...
        arena.bump = *ctx.bumps.get("arena").ok_or(ArenaErr::MissingBump)?;
        Ok(())
    }
    ...
    pub fn init_battle(
        ...
        battle.bump = *ctx.bumps.get("battle").ok_or(ArenaErr::MissingBump)?;
        Ok(())
    }
    ...
    pub fn place_bet(ctx: Context<PlaceBet>, side: u8, amount_lamports: u64) -> Result<()> {
        ...
        let battle = &mut ctx.accounts.battle;
        let now = Clock::get()?.unix_timestamp;
        require!(now < battle.end_ts, ArenaErr::RoundEnded);

        // transfer from bettor to battle PDA (escrow)
        transfer_lamports(&ctx.accounts.bettor, &ctx.accounts.battle, amount_lamports)?;
        ...
        bet.side = side;
        bet.amount = bet.amount.checked_add(amount_lamports).ok_or(ArenaErr::Overflow)?; // allow topping up
        bet.claimed = false;
        bet.bump = *ctx.bumps.get("bet").ok_or(ArenaErr::MissingBump)?;
        Ok(())
    }
    ...
    pub fn claim_bet(ctx: Context<ClaimBet>) -> Result<()> {
        ...
        // Compute rake and distributable pool
        let total_pool = winner_pot
            .checked_add(loser_pot)
            .ok_or(ArenaErr::Overflow)?;
        let rake = total_pool
        ...
        // User share = (bet.amount / winner_pot) * distributable
        ...
        // Send rake to reserve vault once per first claim? To avoid race/dup, we take rake lazily
        // by deducting from distributable; the rake never leaves the battle PDA. Admin can sweep it.
        // (Optional improvement: mark rake_swept flag and transfer to reserve here.)

        // Transfer payout from battle PDA to bettor
        transfer_lamports_signed(
            ...
            &[
                &[BATTLE_SEED,
                    ctx.accounts.arena.key().as_ref(),
                    ctx.accounts.forge_a.key().as_ref(),
                    ctx.accounts.forge_b.key().as_ref(),
                    &battle.nonce.to_le_bytes(),
                    &[battle.bump]
                ],
            ],
        )?;
    ...
    pub fn forge_new_token(
        ...
        forge.mint = ctx.accounts.mint.key();
        forge.creator = ctx.accounts.creator.key();
        forge.bump = *ctx.bumps.get("forge").unwrap();
        forge.created_at = Clock::get()?.unix_timestamp;

        // ----- create mint account at PDA (owner = Token program) -----
        let mint_len = anchor_spl::token::spl_token::state::Mint::LEN as u64;
        let lamports = Rent::get()?.minimum_balance(mint_len as usize);
...
pub struct CreateArena<'info> {
    pub authority: Signer<'info>, // platform admin or delegated arena curator
    #[account(seeds = [GLOBAL_SEED], bump = global.bump)]
    pub global: Account<'info, Global>,
    /// CHECK: oracle signer allowed to update metrics
...
// ------------------------------ Utils ---------------------------------------
fn transfer_lamports(from: &Signer, to: &AccountInfo, amount: u64) -> Result<()> {
...
fn transfer_lamports_signed(
    ...
    let ix = system_instruction::transfer(from.key, to.to_account_info().key, amount);
    invoke_signed(
        &ix,
